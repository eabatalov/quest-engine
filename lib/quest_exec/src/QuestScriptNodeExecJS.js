...
QuestScriptNodeExecJS.prototype.jsFuncNodeExec = function(node, env) {
    this.jsFuncExec(node.getProp('name'), node.getProp('rollbackName'),
        node.getProp('source'), node.getProp('params'), env);
};

QuestScriptNodeExecJS.prototype.jsFuncExec =
function(name, rollbackName, source, paramsFuncNode, env) {
    var funcName = env.isRollbackExec ? rollbackName : name;
    assert(funcName);
