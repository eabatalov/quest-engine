...
QRScriptReverseInterp.prototype.step = function(questEvent) {
    ...
    if (addStartSeq) {
        actions.unshift(QRAction.genCmdSeqStart(action));
    }
    ...
    switch(node.getType()) {
        case _QUEST_NODES.FUNC_CALL:
            if ((node.getProp('source') === SEFuncCallNode.sources.c2) &&
                ...
                action.params = node.getProp('params');
            } else {
                //We don't rollback js functions and pure C2 functions
