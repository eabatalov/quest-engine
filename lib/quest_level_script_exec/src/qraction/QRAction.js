...
QRAction.genNoAction = function(srcAction) {
    var action = new QRAction(_QR_ACTION_TYPES.NO_ACTION);
    action.setStageName(srcAction.getStageName());
    action.setContinuation(srcAction.getContinuation());
    action.setCanReverse(srcAction.getCanReverse());
    action.setHasNext(srcAction.getHasNext());
    return action;
};
