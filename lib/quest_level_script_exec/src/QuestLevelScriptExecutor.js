 ...
 * WARN: Queues are intended for processing of single atomic
 * QRAction sequence.
...
function QuestLevelScriptExecutor(questLevel) {
    ...
    this._questEventsQueue.onEmpty(this, this._onComQueueEmpty);
    this._qrActionsQueue = new CommandQueue();
    this._qrActionsQueue.onCommandPending(this, this._onQRActionPending);
    this._qrActionsQueue.onEmpty(this, this._onComQueueEmpty);
...
//No quest events and QRActions are pending.
QuestLevelScriptExecutor.prototype.isIdle = function() {
    return this._questEventsQueue.isEmpty() &&
        this._qrActionsQueue.isEmpty();
};

QuestLevelScriptExecutor.prototype._onComQueueEmpty = function() {
    if (this.isIdle()) {
        this.events.idle.publish();
    }
};
