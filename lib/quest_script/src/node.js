...
SENode.load = function(savedData, allCondsMap) {
    ...
    var node = SENodeFabric(savedData.type);
...
SENode.prototype.copy = function() {
    //XXX rather shitty. Conditions are ommited.
    var savedData = this.save();
    savedData.inCondIds = {};
    savedData.outCondIds = {};
    //Convert to JSON to loose references to original node data
    savedDataStr = JSON.stringify(savedData);
    savedData = JSON.parse(savedDataStr);
    var nodeCopy = SENode.load(savedData, {});
    nodeCopy.id = SENode.idCnt++;
    return nodeCopy;
};
...
SENode.prototype.setProp = function(name, value) {
    assert(this.props.hasOwnProperty(name),
        name + " property should be defined for current node type");
    this.props[name] = value;
    this.events.propChanged.publish(name, this);
};

SENode.prototype.getProp = function(name) {
    assert(this.props.hasOwnProperty(name),
        name + " property should be defined for current node type");
...
SEStageNode.prototype.takeFromPool = function(objId) {
    ...
    this.events.propChanged.publish("objPool", this);
    return true;
};
...
SEStorylineNode.prototype.addObject = function(objId, stageNode) {
    ...
    this.events.propChanged.publish("objs", this);
};
...
function SEPhraseNode() {
    ...
    this.props.phraseSize = _QUEST_PHRASE_SIZES.LARGE;
...
// ============ FUNC_CALL  ============
function SEFuncCallNodeParameter(type, name, value) {
    this.type = type;
    this.name = name;
    this.value = value;
}

SEFuncCallNodeParameter.types = {
    'str' : 'str',
    'num' : 'num'
};

function SEFuncCallNode() {
    SENode.call(this, _QUEST_NODES.FUNC_CALL);
    this.props.name = "";
    this.props.rollbackName = null;
    this.props.source = SEFuncCallNode.sources.c2;
    this.props.params = [];
    this.props.isPure = false;
}

SEFuncCallNode.sources = {
    'c2' : 'c2',
    'js' : 'js'
};

SEFuncCallNode.prototype = new SENode(_QUEST_NODES.FUNC_CALL);

// ============ NOTIFICATION =============
function SENotificationNode() {
    SENode.call(this, _QUEST_NODES.NOTIFICATION);
    this.props.text = "";
};

SENotificationNode.prototype = new SENode(_QUEST_NODES.NOTIFICATION);

// ============ PLAYER_MOVEMENT =============
function SEPlayerMovementNode() {
    SENode.call(this, _QUEST_NODES.PLAYER_MOVEMENT);
    this.props.enabled = true;
};

SEPlayerMovementNode.prototype = new SENode(_QUEST_NODES.PLAYER_MOVEMENT);
...
function SENodeFabric(nodeType) {
    switch(nodeType) {
        ...
        case _QUEST_NODES.FUNC_CALL:
            return new SEFuncCallNode();
        break;
        case _QUEST_NODES.NOTIFICATION:
            return new SENotificationNode();
        break;
        case _QUEST_NODES.PLAYER_MOVEMENT:
            return new SEPlayerMovementNode();
        break;
    }
}
