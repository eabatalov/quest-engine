function StoryLineInterpretator(storyLineNode) {
    ...
    //Single cond from stage node always exists
    this.currentCond = storyLineNode.getInConds()[0];
}
...
StoryLineInterpretator.prototype._setCurrentCond = function(cond) {
    this.currentCond = cond;
};

StoryLineInterpretator.prototype.getCurrentCond = function() {
    return this.currentCond;
};

/*
 * Is node sequence in interpretation progress.
 */
StoryLineInterpretator.prototype.isSeqInProgress = function() {
    return this.getCurrentNode().getContinue();
};
...
StoryLineInterpretator.prototype.step = function(questEvent) {
    ...
    if (questEvent.getType() === _QUEST_EVENTS.RECALL_LAST_NODE)
        return this.getCurrentNode();

    var eventCondType = StoryLineInterpretator.
        questEventTypeToCondType(questEvent.getType());
