...
QuestGame.bootstrap = function(config) {
    ...
    if (config.levelReplaySaver) {
        createLevelReplaySaverCode = "var levelReplaySaver = new "
            + config.levelReplaySaver.className + "(";
        for(var paramIx = 0; paramIx < config.levelReplaySaver.params.length; ++paramIx) {
            createLevelReplaySaverCode +=
                "eval(config.levelReplaySaver.params[" + paramIx.toString() + "]),";
        };
        createLevelReplaySaverCode += "null);"; //fix comma
    } else {
        createLevelReplaySaverCode = "var levelReplaySaver = null;";
    }

    if (config.levelReplayLoader) {
        createLevelReplayLoaderCode = "var levelReplayLoader = new "
            + config.levelReplayLoader.className + "(";
        for(var paramIx = 0; paramIx < config.levelReplayLoader.params.length; ++paramIx) {
            createLevelReplayLoaderCode +=
                "eval(config.levelReplayLoader.params[" + paramIx.toString() + "]),";
        };
        createLevelReplayLoaderCode += "null);"; //fix comma
    } else {
        createLevelReplayLoaderCode = "var levelReplayLoader = null;";
    }

    eval(createPersistentStorageCode);
    eval(createInAppPurchaseProviderCode);
    eval(createLevelLoaderCode);
    eval(createLevelReplaySaverCode);
    eval(createLevelReplayLoaderCode);
